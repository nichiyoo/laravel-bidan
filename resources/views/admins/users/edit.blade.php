<x-app-layout>
    <x-heading level="h2">
        <x-slot name="title">
            {{ __('Perbarui Data Pengguna') }}
        </x-slot>

        <x-slot name="description">
            {{ __('Perbarui Data Pengguna, dan pastikan data yang dikirim benar') }}
        </x-slot>
    </x-heading>

    <form action="{{ route('admins.users.update', $user) }}" method="post" class="grid gap-6 lg:grid-cols-2">
        @csrf
        @method('PATCH')
        ...
        <div>
            ...
            <x-select id="role" name="role" required>
                @foreach ($roles as $key => $value)
                    <option value="{{ $key }}" @if ($key == (old('role') ?? $user->role)) selected @endif>
