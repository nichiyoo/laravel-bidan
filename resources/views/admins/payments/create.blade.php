<x-app-layout>
    <x-heading level="h2">
        <x-slot name="title">
            {{ __('Tambah Data Metode Pembayaran') }}
        </x-slot>

        <x-slot name="description">
            {{ __('Tambah Data Metode Pembayaran, dan pastikan data yang dikirim benar') }}
        </x-slot>
    </x-heading>

    <form action="{{ route('admins.payments.store') }}" method="post" class="grid gap-6 lg:grid-cols-2">
        @csrf
        ...
        <div>
            ...
            <x-input id="account" type="text" name="account" placeholder="{{ __('payments.account.placeholder') }}"
                value="{{ old('account') }}" autocomplete="account" autofocus required />
